...
// Captures section previews from $SITE_URL (default https://hansendong.top)
// into public/screenshots/. Uses playwright's bundled chromium.
...
try {
  ...
  if (!resp || !resp.ok()) {
    throw new Error(`Page returned ${resp ? resp.status() : 'no response'}`);
  }
  ...
  for (const { sel, file } of SECTIONS) {
    ...
    if (!exists) { console.warn(`⚠ ${sel} not found, skipping ${file}`); continue; }
